...
4. タイトル画面構成
================================
LINKED BLOCKS_ と同一のレイアウト構成。
全面マトリックス背景の上に、タイトルロゴとUIパネルを配置。

タイトルロゴ（画面上部）：
・「FALL」+改行+「DODGE」ネオングリーン発光（2行表示）
・text-shadow多重（3層グロー）、大きく目立つ
・ロゴ下に4色の3D棒人間イラスト（P1ピンク/P2シアン/P3イエロー/P4パープル）が横並び

中央UIパネル（角括弧装飾付きボックス）：
LINKED BLOCKS_ と同じくネオン枠+角ブラケット装飾で囲む。
上から縦並びで以下の要素を配置:

1) USER NAME ラベル
2) ユーザーネーム入力欄（プレースホルダー:「名前を入力...」）
3) 登録してゲームを始める ← メインボタン（ネオン枠）
4) 登録しないでゲームを始める ← サブボタン（薄い枠）
   ---（余白）---
5) 一人でプレイ（CPU対戦） ← ネオン枠
6) マルチプレイ（最大4人） ← ネオン枠
   ---（余白）---
...
フッター帯（パネル外下部）：
・BGM ON/OFF + SE ON/OFF（小さいアイコン or テキスト）
...
・誰か1人でも被弾 → 全員ゲームオーバー
・スコアは全プレイヤーの合計
・協力ランキングに登録（参加メンバー名+合計スコア）
・妨害アイテムは出現しない（パワーアップは全員共通で恩恵）
・マルチ2〜4人専用
...
const GAME_MODES = {
    ...
    coop:      { label: '協力モード',     solo: false, multi: true, maxPlayers: 4, minPlayers: 2 },
...
20. マルチプレイ仕様
================================
■ ルーム管理（LINKED BLOCKS_ 同等方式）
・ルームID: 6文字ランダム英数字（自動生成）
・最大4人
・1ID = 1ルーム
・ルームURL: https://[host]/FALL_DODGE/?room=XXXXXX
・URLをコピーして友達に共有 → 相手がURLを開くと自動でルーム参加
・ルームIDを直接入力して参加も可能

■ ロビー画面（モーダル形式 — LINKED BLOCKS_ 同等）
タイトル画面で「マルチプレイ（最大4人）」押下時にモーダル表示。
...
| モード     | ボタン表示 | 人数    | ルール                                  |
|-----------|----------|--------|----------------------------------------|
| 個人戦     | 個人戦   | 2〜4人  | 最後の1人が勝利。勝者はランキング登録        |
| チーム戦   | チーム戦  | 4人必須 | P1+P3 vs P2+P4。チーム全滅で負け          |
| 協力      | 協力     | 2〜4人  | 全員で生存。1人被弾→全員終了。合計スコア競う  |

・デフォルト: 個人戦
・チーム戦は4人揃わないとゲーム開始不可（ロビーで警告表示）

■ マッチングフロー（LINKED BLOCKS_ 同等）
1. モーダルでルームIDを入力（空欄なら自動生成）
2. 対戦モード・難易度を選択
3. 「ルームに参加」押下
4. 同じルームIDを入力した他のプレイヤーと自動マッチング
5. ロビー画面（待機画面）に遷移
6. 全員揃ったらホストが「ゲーム開始」

■ ロビー待機画面（#lobby-screen）
```
ONLINE ROOM // ルームID: XXXXXX
LINK: https://[host]/FALL_DODGE/?room=XXXXXX  [コピー]
モード: 個人戦 / 難易度: 普通
...
■ 個人戦ランキング
・個人戦で最後まで生き残った勝者のスコアをランキングに登録
・ランキング表示は LINKED BLOCKS_ と同じフォーマット
・Firebase: multiRankings/<pushId> = { username, score, mode: 'individual', players, timestamp }
・ランキング画面で「ソロ / マルチ個人戦 / 協力」タブ切替

```javascript
// 個人戦勝利時のランキング登録
function registerMultiWin(winnerName, score, playerCount) {
    database.ref('multiRankings').push({
        username: winnerName,
        score: score,
        mode: 'individual',
...
| モード        | 人数      | 内容                                  |
|-------------|----------|--------------------------------------|
| ノーマル対戦   | 2〜4人   | 最後まで生き残った1人が勝利              |
| チーム戦(2v2) | 4人必須   | P1+P3 vs P2+P4、チーム全滅で負け       |
| 協力モード    | 2〜4人   | 全員で生存時間を競う。1人でも被弾→全員終了 |
| タイムアタック | 2〜4人   | 60秒間のスコア合計で競う               |
| ボスラッシュ  | 2〜4人   | 巨大落下物10体を全員で回避              |

■ 協力モード詳細
・全員が同じフィールドで回避
・誰か1人が被弾すると全員ゲームオーバー
・スコアは全員の合計
・協力ランキングに登録（チーム名+合計スコア）
・Firebase: coopRankings/<pushId> = { players: [...], score, timestamp }
...
■ 勝敗判定
・対戦: 最後まで alive: true のプレイヤーが勝利
・協力: 全員 alive で可能な限り長く生存
・残り1人になった時点でGAME SET
...
Phase 6: マルチプレイ
 28. ロビー画面（4人対応）+ ルーム作成/参加 + URL共有機能
 29. SyncLayer実装
...
・ダッシュ・コンボによる奥深い操作性
・8つのゲームモード（対戦+協力+シークレット）によるリプレイ性
